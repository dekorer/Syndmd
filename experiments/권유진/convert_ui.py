...
class Worker(QObject):
    ...
    # [수정] __init__에서 더 많은 정보를 받도록 변경
    def __init__(self, main_window, text_content, template_path, template_page):
        super().__init__()
    ...
    def run(self):
        """이 함수가 백그라운드 스레드에서 HWP 생성부터 PDF 변환까지 모두 처리합니다."""
        temp_hwp_path = ""
        try:
            # --- 1단계: 임시 HWP 파일 생성 ---
            ...
            # converter_test.py를 호출하여 빈 HWP 파일 생성
            ...
            self.progress.emit(50) # HWP 생성 완료
            
            # --- 2단계: HWP -> PDF -> QPixmap 변환 ---
            # 기존 generate_preview_pixmap의 로직을 그대로 가져옴
            pixmap = self.main_window.generate_preview_pixmap(temp_hwp_path)
            
            self.progress.emit(100) # 모든 작업 완료
            self.finished.emit(pixmap, None)

        except Exception as e:
            # 어떤 단계에서든 오류가 발생하면 에러를 전달
            self.finished.emit(QPixmap(), e)
        finally:
            # --- 3단계: 임시 파일 정리 ---
            # 작업이 성공하든 실패하든 임시 파일은 항상 삭제
            if temp_hwp_path:
                self.main_window.cleanup_temp_files(temp_hwp_path)
